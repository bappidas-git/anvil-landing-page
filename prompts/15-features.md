# 15 — Features Data + Features Section

## Objective
Replace the feature groups (currently "Techniques / Patient Care / Experience & Trust") with Anvil-aligned groups that communicate technology, service, and trust advantages.

## Files to modify
- `src/data/featuresData.js`
- `src/components/sections/FeaturesSection/FeaturesSection.jsx`
- `src/components/sections/FeaturesSection/FeaturesSection.module.css` (hex cleanup)

## Target groups
1. **Solar Technology** — what sits on your roof
2. **Full-Service Install** — how Anvil takes care of you
3. **Trust & Warranty** — why to pick Anvil

## Implementation — `src/data/featuresData.js`

```js
export const featuresData = [
  {
    category: "Solar Technology",
    items: [
      {
        icon: "mdi:solar-panel-large",
        title: "Tier-1 Monocrystalline Panels",
        description:
          "High-efficiency panels from globally-certified manufacturers with 25-year linear power output warranty.",
      },
      {
        icon: "mdi:chip",
        title: "Smart Inverters",
        description:
          "Tier-1 inverters with built-in MPPT, grid protection, and app-based live monitoring of every kilowatt-hour you generate.",
      },
      {
        icon: "mdi:battery-charging-high",
        title: "Lithium-ion Battery Backup",
        description:
          "Optional high-density lithium batteries with 10-year life for power-cut-proof hybrid systems.",
      },
    ],
  },
  {
    category: "Full-Service Install",
    items: [
      {
        icon: "mdi:handshake",
        title: "Dedicated Anvil Saathi",
        description:
          "One person owns your entire journey — site survey, design, paperwork, install, and support.",
      },
      {
        icon: "mdi:file-document-check",
        title: "PM Surya Ghar Paperwork",
        description:
          "Application, DISCOM approval, net metering, and subsidy disbursal handled end-to-end.",
      },
      {
        icon: "mdi:bank",
        title: "Zero Down-Payment EMI",
        description:
          "Solar loans from 7% p.a. with tenors up to 10 years, on-boarded through our partner banks.",
      },
    ],
  },
  {
    category: "Trust & Warranty",
    items: [
      {
        icon: "mdi:shield-sun",
        title: "25-Year Panel Warranty",
        description:
          "Linear power output warranty directly from the OEM — matched by Anvil's 5-year workmanship warranty.",
      },
      {
        icon: "mdi:map-marker-radius",
        title: "PAN-India Service Network",
        description:
          "Vetted installer teams across 300+ cities. Your Saathi is never more than a day away.",
      },
      {
        icon: "mdi:chart-line",
        title: "Live Generation Tracking",
        description:
          "The Anvil app shows your live output, savings, and CO₂ offset — with monthly performance reports.",
      },
    ],
  },
];
```

## FeaturesSection.jsx updates
1. Update the section eyebrow / title / subtitle:
```jsx
<SectionTitle
  eyebrow="Built for India"
  title="Better solar. Less hassle."
  subtitle="Every Anvil installation is engineered, warrantied, and supported by a team that actually picks up the phone."
/>
```
2. Do not change the mapping logic — `featuresData.map(group => ...)` should keep working.
3. Any hardcoded "Explore all features" CTA → update label to `"Book Your Free Call"` and wire to `handleOpenLeadForm`.

## Guardrails
- Keep `featuresData` export name identical
- Keep `category`, `items`, `icon`, `title`, `description` keys identical
- Do not change the section's DOM structure

## Validation checklist
- [ ] Three groups exist: Solar Technology, Full-Service Install, Trust & Warranty
- [ ] Each group has exactly 3 items with icon/title/description
- [ ] No medical terminology remains
- [ ] Icons use `mdi:*` namespace
- [ ] Section CTA opens the lead form

## Next step
Proceed to `16-stats.md` to rewrite the stats data + StatsSection.
